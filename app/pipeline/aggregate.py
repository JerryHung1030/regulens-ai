from typing import List, Optional  # Dict not used here but often useful
import numpy as np 
...
def aggregate_assessments_for_pair(
    ...
    for ta in assessments_to_summarize:
        if detailed_lines_count >= max_detailed_lines and assessments_to_summarize is triple_assessments:
            # Ensure this line is also formatted as a list item if it's part of the list
        ...
        # Corrected multi-line f-string for Markdown list item
        evidence_summary = f"- {status_icon} **Evidence:** `{ta.evidence_doc_id}` (Chunk: `{ta.evidence_chunk_id}`)\n  - **Status:** {ta.status}\n  - **Analysis:** {analysis_text}"
        if suggestion_text != "N/A":  # Only add suggestion line if it exists
            evidence_summary += f"\n  - **Suggestion:** {suggestion_text}"
        
        summary_lines.append(evidence_summary)
        detailed_lines_count += 1
            
    summary_analysis_str = "\n".join(summary_lines)  # Join with single newline.
